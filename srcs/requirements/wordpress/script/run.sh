...
if [ ! -f "/var/www/html/wp-config.php" ]; then
	...
	wp config create --dbhost ${DB_HOST} --dbname ${MYSQL_DATABASE} --dbuser ${MYSQL_USER} --dbpass ${DB_PASSWORD} --allow-root
	wp core install --url ${DOMAIN_NAME} --admin_user ${ADMIN_USER} --admin_email ${ADMIN_EMAIL} --admin_password ${ADMIN_PASSWORD} --skip-email --allow-root
